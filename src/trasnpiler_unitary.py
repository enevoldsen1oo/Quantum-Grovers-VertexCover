...
class VCPToCircuitTranspilerUnitary:
    def __init__(self, problem: VertexCoverProblem):
        ...
        self.num_qubits = self.num_vertices + self.num_counting_qubits + self.num_edges + 1 # value qubits + ancilla + counting qubits + edge qubits + phase kickback qubit
    ...
    def decrement(self, qc: QuantumCircuit, control: int):
        counting_qbits = list(range(self.num_vertices, self.num_vertices + self.num_counting_qubits))
    ...
    def increment(self, qc: QuantumCircuit, control: int):
        counting_qbits = list(range(self.num_vertices, self.num_vertices + self.num_counting_qubits))
        for i in range(len(counting_qbits)):
            qc.append(MCXGate(len(counting_qbits)-i), [control] + counting_qbits[:len(counting_qbits)-i])
    ...
    def edge_constraint(self, qc: QuantumCircuit):
        ...
        for i, (v1, v2) in enumerate(self.edges):
            qc.ccx(v1, v2, self.num_vertices + self.num_counting_qubits + i)
    ...
    def phase_kickback(self, qc: QuantumCircuit):
        ...
        flip_qubits = [i + self.num_vertices for i, bit in enumerate(target_count_bits) if bit == '0']
